...
const zh = {
  ...
  pluginIntro: "从指定文件夹随机抽取笔记，支持标签和属性条件组筛选，全屏逐题浏览，支持折叠 Callout 显示/隐藏答案；内置测验模式（计时器、正确/错误/跳过标记、答题统计），可将抽取结果导出为 HTML（交互式）、Markdown、纯文本或 Word 文档。适合闪卡复习、随机回顾和模拟抽题。",
  usageTitle: "使用方法",
  usageItems: [
    "在「抽取控制」中选择目标文件夹，并按需设置标签、排除文件夹、属性条件组等筛选规则",
    "通过命令面板、Ribbon 图标或右键文件夹启动复习",
    "用 ← → 或空格切换题目，按 A 显示/隐藏答案，J/K 标记正确/错误，Esc 退出",
    "复习时可在顶栏点击「历史」查看右侧边栏答题统计，鼠标悬停记录可点击「回顾」分屏打开原笔记",
...
const en = {
  ...
  pluginIntro: "Randomly pick notes from tag and property filtering, browse fullscreen with answer toggle, quiz mode with timer and scoring, and export to HTML (interactive), Markdown, plain text, or Word. Ideal for flashcard review, random browsing, and quiz simulation.",
  usageTitle: "Usage",
  usageItems: [
    "In \"Extraction Control\", pick a target folder and configure tags, excluded folders, and property condition groups",
    ...
    "Click \"History\" in the top bar to open the right sidebar with quiz stats; hover over a record and click \"Review\" to open the note in a split pane",
